...
class S4_model(model_base):
    def __init__(self, config, params=None, iscl=True, lmin=30):
    ...
    def model_spec(self, lmax=None):
        """
        Return model spectra in K-squared units.

        The output is either ``Cl`` or ``l * (l + 1) * Cl / (2 * pi)``.
        """
        #
        ...
        ms['cmbl']['BB'] = params['A_L'] * (self.lencl['B-mode C_l'][: lmax + 1] * lcmb(ell))

        ms['cmbt']['BB'] = params['r'] / camb_r * (self.tencl['B-mode C_l'][: lmax + 1] * lcmb(ell))

        ms['dust']['BB'] = params['A_d'] * (ell / params['fg_pivot']) ** params['alpha_d'] * lfg(ell)
        ms['dust']['EE'] = ms['dust']['BB'] * params['EB_d']

        ms['sync']['BB'] = params['A_s'] * (ell / params['fg_pivot']) ** params['alpha_s'] * lfg(ell)
...
class BK_model(model_base):
    def __init__(self, params=None, liketheorydir="/sdf/home/w/wlwu/bk/"):
        self.liketheorydir = liketheorydir
        super().__init__(params=params)

    def get_default_model_params(self):
        """
        Return the default BK model parameters.

        Notes
        -----
        ``r`` is the tensor-to-scalar ratio. ``A_L`` is the lensing amplitude,
        with 1 corresponding to standard LCDM lensing. ``A_s`` and ``A_d`` are
        synchrotron and dust power-spectrum amplitudes in ``uK_CMB^2`` at the
        specified frequency and ell pivots. ``beta_s`` and ``beta_d`` are
        frequency spectral indices, while ``alpha_s`` and ``alpha_d`` are
        spatial spectral indices. ``EB_s`` and ``EB_d`` are E/B ratios, where 1
        means equal E and B power and 2 means twice as much E as B. ``epsilon``
        is the dust/sync correlation parameter. ``T_d`` is the dust temperature,
        19.6 K for the PIP97 model.

        Additional parameters are put in ``likedata.opt`` through
        ``like_read_theory`` in BK: ``sync_freq`` for ``A_s``, ``dust_freq`` for
        ``A_d``, and ``fg_pivot`` for the ``A_s`` and ``A_d`` ell pivot.
    ...
    def model_spec(self, lmax=None):
        """
        Compute the model spectra from ``model_rms.m``.

        Leave ``n_exptfreq`` scaling to the BPCM code.

        Returns
        -------
        model_spec
            Dictionary of unbinned spectra. Component keys are ``cmbl``,
            ``dust``, ``sync``, and ``cmbt``. Spectrum keys are ``TT``, ``EE``,
            and ``BB``. Outputs are in ``l * (l + 1) * Cl / (2 * pi)``, matching
            the dust/sync modeling.
        """
