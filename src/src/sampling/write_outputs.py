...
def reorder(bpcm, spec):
    '''
    reorder array (n_spec, nbins) with ordering
        11 22 33 44
           12 13 14
              23 24
                 34
...
def dataset(cf, bpcm, fname, fname_simn=None, decorr="lin"):
    """
    dataset defintion
    file names of Nl, Clhat ...

    inputs: 
      cf: config file
      fname_simn = fname_base if read data
  

    output: .dataset
    """
    if fname_simn is None: fname_simn = fname

    f = open(fname_simn+'.dataset', 'wt')
    f.write('like_approx = HL \n')

    mapnames = "".join(['{}_{} '.format(freq,field) for freq in bpcm.exptfreq
                        for field in bpcm.fields])
    mapfield = "".join(['{} '.format(field) for i in bpcm.exptfreq
                        for field in bpcm.fields])
    f.write('map_names = %s \n'%mapnames)
...
def append_dataset(ilc_bands, fname, fname_simn=None):
    """
    for ILC bandpowers; need input map bands 
    """
    if fname_simn is None: fname_simn = fname
    print(fname_simn)
...
def Cfl(cf, bpcm, fname, scale_cfl=1.0):
    """
    fiducial model bandpowers for H-L likelihood
    output text file in format:
    bin_n, n_spec of bandpowers

    scale_cfl converts between K^2 to uK^2
    
    ###FIXME: duplicate code in scale_bpcm()
    ### BPCM model must be same as Cfl model.
    """
...
def Cfl_b1(cf, bpcm, fname, nmaps):
    ...
    ell = np.arange(bpcm.theory_lmax+1); ell[0]=1
    lpivot = 80
    ...
    if nmaps==3:
        ...
        Cfl = np.vstack([cmbxcmb, dusxdus, synxsyn, ltxlt, 
                         cmbxdus, dusxsyn, synxlt, cmbxsyn, 
                         dusxlt, cmbxlt]).transpose()
...
def Clhat(cf, bpcm, fname, Clhat):
    ...
    sim bandpowers in (n_spec, all_nbins, nsim)
    Clhat passes (n_spec, nbins), in correct order 00 11 ... 01 12.. 02 13
    '''
...
def bpcm(cf, bpcm, fname, covmat):
    '''
    input covmat (n_spec*nbins, n_spec*nbins)
...
def get_bpstr(cf, bpcm):
    ...
    bpstr = "".join(['{}_{}x{}_{} '.format(mo[order[s]['e1']][0],
                                     order[s]['f1'],
                                     mo[order[s]['e2']][0],
                                     order[s]['f2'])
                    for s in order])
